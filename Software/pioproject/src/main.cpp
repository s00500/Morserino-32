...
///// its is crucial to have the right board version - Boards 2 and 2a
///(prototypes only) set it to 2, Boards 3 set it to 3
///// the Board Version 2 is for HEltec Modules V1 only, Board Version 3 for
/// Heltec V2 only
...
/////// the first version of the CW over LoRA protocol; future versions will be
/// B02, B03, B00 (reserved for future use)

#define CWLORAVERSION B01

/******************************************************************************************************************************
 *  morse_3 Software for the Morserino-32 multi-functional Morse code machine,
 *based on the Heltec WiFi LORA (ESP32) module *** Copyright (C) 2018  Willi
 *Kraml, OE1WKL ***
 *
...
/*****************************************************************************************************************************
 *  code by others used in this sketch, apart from the ESP32 libraries
 *distributed by Heltec (see:
 *https://github.com/Heltec-Aaron-Lee/WiFi_Kit_series)
 *
 *  ClickButton library -> https://code.google.com/p/clickbutton/ by Ragnar
 *Aronsen
 *
 *
 *  For volume control of NF output: I used a similar principle as  Connor
 *Nishijima, see
 *                                   https://hackaday.io/project/11957-10-bit-component-less-volume-control-for-arduino
 *                                   but actually using two PWM outputs,
 *connected with an AND gate Routines for morse decoder - to a certain degree
 *based on code by Hjalmar Skovholm Hansen OZ1JHM
...
// DialogInput_plain_12, DialogInput_bold_12 & DialogInput_plain_15,
// DialogInput_bold_15 these fonts were created with this tool:
...
// SENS_FACTOR is used for auto-calibrating sensitivity of touch paddles
// (somewhere between 2.0 and 2.5)
#define SENS_FACTOR 2.22

///////////////////////////////      H A R D W A R E
////////////////////////////////////////////// / Here are the definitions for
/// the various hardware-related I/O pins of the ESP32
/////////////////////////////////////////////////////////////////////////////////////////////////////
///// Board versions:
///// 2(a): for Heltec ESP32 LORA Version 1 (Morserino-32 prototypes)
///// 3: for heltec ESP32 LORA Version 2
///// Warning: Board version 1 not supported anymore!!!! (was an early
/// prototype)
/////
///// the following Pins are dependent on the board version
///// 21:  V2: an encoder port, using interrupts      V3: Vext - used internally
/// to switch Vext on and off
///// 39:  V2: ADC input to measure battery voltage   V3: encoder port
///(interrupt driven) instead of 21
///// 13:  V2: leftPin (external paddle, ext. pullup) V3: used internally to
/// read battery voltage
///// 38:  V2: not in use                             V3: encoder port
///(interrupt driven) instead of 35
///// 32:  V2: used for LoRa internally               V3: leftPin
///// 33:  V2: used for LoRa internally               V3: rightPin
///// 34: rightPin                                    V3: cannot be used any
/// longer, used for LoRa internally
///// 35: PinDT                                       V3: cannot be used any
/// longer, used for LoRa internally

#ifndef BOARDVERSION
#error \
    "You need to define a board version  at the beginning of the source file!"
#endif
...
const int leftPin =
    13;  // external pullup resistor is necessary for closing contacts!
const int rightPin =
    34;  // external pullup resistor is necessary for closing contacts!

/// where is the encoder?
const int PinCLK = 21;  // Used for generating interrupts using CLK signal -
                        // needs external pullup resisitor!
const int PinDT =
    35;  // Used for reading DT signal  - needs external pullup resisitor!
...
/// where are the external paddles?
const int leftPin =
    33;  // external pullup resistor is necessary for closing contacts!
const int rightPin =
    32;  // external pullup resistor is necessary for closing contacts!

/// where is the encoder?
const int PinCLK = 38;  // Used for generating interrupts using CLK signal -
                        // needs external pullup resisitor!
const int PinDT =
    39;  // Used for reading DT signal  - needs external pullup resisitor!
...
#else
#error \
    "Invalid Board Version! This program version only supports board versions 2 and 3."
#endif

//////// HARDWARE definitions that have not been changed between board version2
/// and 3 (ESP32 WIFI LORA V.1 and V.2)

// OLED pins to ESP32 GPIOs:

// define OLED display and its address - the Heltec ESP32 LoRA uses its display
// on 0x3c
...
//// with the following we define which pins are used as output for the two pwm
/// channels / HF output (with varying duticycle and fixed frequency) and LF
/// output (with varying frequency and fixed dutycycle of 50%)
/// are being added with a 2-transistor AND gate to create a tone frequency with
/// variable frequency and volume
...
// Tx keyer
const int keyerPin = 25;  // this keys the transmitter / through a MOSFET
                          // Optocoupler - at the same time lights up the LED
...
const int modeButtonPin =
    37;  // input pin for mode button - needs external pullup!
...
char textBuffer[NoOfLines]
               [2 * NoOfCharsPerLine +
                1];  /// we need extra room for style markers (FONT_ATTRIB
                     /// stored as characters to toggle on/off the style within
                     /// a line) and 0 terminator
...
#define lora_width \
  6  /// a simple logo that shows when we operate with loRa, stored in XBM
     /// format
#define lora_height 11
static unsigned char lora_bits[] = {0x0f, 0x18, 0x33, 0x24, 0x29, 0x2b,
                                    0x29, 0x24, 0x33, 0x18, 0x0f};

/////////////////////// parameters for LF tone generation and  HF (= vol ctrl)
/// PWM
int toneFreq = 500;
int toneChannel = 2;  // this PWM channel is used for LF generation, duty cycle
                      // is 0 (silent) or 50% (tone)
int lineOutChannel = 3;  // this PWM channel is used for line-out LF generation,
                         // duty cycle is 0 (silent) or 50% (tone)
int volChannel = 8;  // this PWM channel is used for HF generation, duty cycle
                     // between 1% (almost silent) and 100% (loud)
...
const int encoderWaitTime =
    100;  // how long to wait for next reading from encoder in microseconds
...
#define IAMBICB \
  2  // Curtis Mode B (with enhanced Curtis timing, set as parameter
...
encoderMode encoderState =
    speedSettingMode;  // we start with adjusting the speed
...
const String prefOption[] = {
    "Encoder Click", "Tone Pitch Hz", "Paddle:      ", "Paddle Polar.",
    ...
    "Koch"};
prefPos keyerOptions[] = {posClicks,           posPitch,
                          posExtPaddles,       posPolarity,
                          posCurtisMode,       posCurtisBDahTiming,
                          posCurtisBDotTiming, posACS};
prefPos generatorOptions[] = {
    posClicks,         posPitch,          posExtPaddles,     posInterWordSpace,
    posInterCharSpace, posRandomOption,   posRandomLength,   posCallLength,
    posAbbrevLength,   posWordLength,     posTrainerDisplay, posWordDoubler,
    posKeyTrainerMode, posLoraTrainerMode};
prefPos playerOptions[] = {
    posClicks,         posPitch,          posExtPaddles,
    posInterWordSpace, posInterCharSpace, posTrainerDisplay,
    posWordDoubler,    posKeyTrainerMode, posLoraTrainerMode};
prefPos echoPlayerOptions[] = {posClicks,           posPitch,
                               posExtPaddles,       posPolarity,
                               posCurtisMode,       posCurtisBDahTiming,
                               posCurtisBDotTiming, posACS,
                               posEchoToneShift,    posInterWordSpace,
                               posInterCharSpace,   posEchoRepeats,
                               posEchoConf};
prefPos echoTrainerOptions[] = {posClicks,           posPitch,
                                posExtPaddles,       posPolarity,
                                posCurtisMode,       posCurtisBDahTiming,
                                posCurtisBDotTiming, posACS,
                                posEchoToneShift,    posInterWordSpace,
                                posInterCharSpace,   posRandomOption,
                                posRandomLength,     posCallLength,
                                posAbbrevLength,     posWordLength,
                                posEchoRepeats,      posEchoConf,
                                posSpeedAdapt};
prefPos kochGenOptions[] = {
    posClicks,         posPitch,        posExtPaddles,     posInterWordSpace,
    posInterCharSpace, posRandomLength, posAbbrevLength,   posWordLength,
    posTrainerDisplay, posWordDoubler,  posKeyTrainerMode, posLoraTrainerMode};
prefPos kochEchoOptions[] = {posClicks,           posPitch,
                             posExtPaddles,       posPolarity,
                             posCurtisMode,       posCurtisBDahTiming,
                             posCurtisBDotTiming, posACS,
                             posEchoToneShift,    posInterWordSpace,
                             posInterCharSpace,   posRandomLength,
                             posAbbrevLength,     posWordLength,
                             posEchoRepeats,      posEchoConf,
                             posSpeedAdapt};
prefPos loraTrxOptions[] = {
    posClicks,           posPitch,      posExtPaddles,
    posPolarity,         posCurtisMode, posCurtisBDahTiming,
    posCurtisBDotTiming, posACS,        posEchoToneShift};
prefPos extTrxOptions[] = {posClicks,           posPitch,
                           posExtPaddles,       posPolarity,
                           posCurtisMode,       posCurtisBDahTiming,
                           posCurtisBDotTiming, posACS,
...
unsigned int interCharacterSpace,
    interWordSpace;   // need to be properly initialised!
unsigned int effWpm;  // calculated effective speed in WpM
unsigned int lUntouched =
    0;  // sensor values (in untouched state) will be stored here
...
GEN_TYPE generatorMode =
    RANDOMS;  // trainer: what symbol (groups) are we going to send? 0 -  5

Preferences pref;  // use the Preferences library for storing and retrieving
                   // objects the preferences variable and their defaults
...
uint8_t p_sidetoneVolume =
    60;  // side tone volume, as a value between 0 and 100   0 -100
bool p_didah = false;  // paddle polarity bool
uint8_t p_keyermode =
    2;  // Iambic keyer mode: see the #defines above        1 -  3
...
bool p_useExtPaddle = false;   // true if we use an external (mechanical)
                               // paddle instead of the touch sensors
uint8_t p_ACSlength =
    0;  // in ACS: we extend the pause between charcaters to the equal length of
        // how many dots (2, 3 or 4 are meaningful, 0 means off) 0, 2-4
bool p_encoderClicks =
    true;  // all: should rotating the encoder generate a click?
uint8_t p_randomLength =
    3;  // trainer: how many random chars in one group -    1 -  5
uint8_t p_randomOption =
    0;  // trainer: from which pool are we generating random characters?  0 - 9
uint8_t p_callLength = 0;    // trainer: max length of call signs generated (0 =
                             // unlimited)    0, 3 - 6
uint8_t p_abbrevLength = 0;  // trainer: max length of abbreviations generated
                             // (0 = unlimited) 0, 2 - 6
uint8_t p_wordLength = 0;  // trainer: max length of english words generated (0
                           // = unlimited) 0, 2 - 6
uint8_t p_trainerDisplay =
    DISPLAY_BY_CHAR;  // trainer: how we display what the trainer generates:
                      // nothing, by character, or by word  0 - 2
uint8_t p_curtisBTiming = 45;  // keyer: timing for enhanced Curtis mode: dah 0
                               // - 100
uint8_t p_curtisBDotTiming = 75;  // keyer: timing for enhanced Curtis mode: dit
                                  // 0 - 100
uint8_t p_interWordSpace = 7;  // trainer: normal interword spacing in lengths
                               // of dit,           6 - 45 ; default = norm = 7

uint8_t p_responsePause = 5;  // in echoTrainer mode, how long do we wait for
                              // response? in interWordSpaces; 2-12, default 4
uint8_t p_echoRepeats = 3;  // how often will echo trainer repeat an erroniously
                            // entered word? 0 - 7, 7=forever, default = 3
uint8_t p_kochFilter = 5;   // constrain output to characters learned according
                            // to Koch's method 2 - 45
bool p_wordDoubler = false;  // in CW trainer mode only, repeat each word
uint8_t p_echoToneShift =
    1;  // 0 = no shift, 1 = up, 2 = down (a half tone)                   0 - 2
bool p_echoConf =
    true;  // true if echo trainer confirms audibly too, not just visually
uint8_t p_keyTrainerMode =
    1;  // key a transmitter in generator and player mode?
        //  0: "Never";  1: "CW Keyer only";  2: "Keyer&Trainer";
uint8_t p_loraTrainerMode =
    0;  // transmit via LoRa in generator and player mode?
        //  0: "No";  1: "yes"
uint8_t p_goertzelBandwidth = 0;  //  0: "Wide" 1: "Narrow"
bool p_speedAdapt =
    false;  //  true: in echo modes, increase speed when OK, reduce when not ok
...
uint32_t p_fileWordPointer = 0;  // remember how far we have read the file in
                                 // player mode / reset when loading new file
uint8_t p_promptPause = 2;  // in echoTrainer mode, length of pause before we
                            // send next word; multiplied by interWordSpace

bool kochActive = false;  // set to true when in Koch trainer mode
...
//
unsigned char keyerControl = 0;  // this holds the latches for the paddles and
                                 // the DIT_LAST latch, see above

bool DIT_FIRST = false;  // first latched was dit?
...
unsigned long charCounter =
    25;  // we use this to count characters after changing speed - after n
         // characters we decide to write the config into NVS
...
byte treeptr = 0;  // pointer used to navigate within the linked list
                   // representing the dichotomic tree

unsigned long interWordTimer = 0;  // timer to detect interword spaces
unsigned long acsTimer =
    0;  // timer to use for automatic character spacing (ACS)
...
// a = CWchars.substring(0,26); 9 = CWchars.substring(26,36); ? =
// CWchars.substring(36,45); <> = CWchars.substring(44,49); a9 =
// CWchars.substring(0,36); 9? = CWchars.substring(26,45); ?<> =
// CWchars.substring(36,50); a9? = CWchars.substring(0,45); 9?<> =
// CWchars.substring(26,50); a9?<> = CWchars;
...
int rxDitLength =
    0;  // set new value for length of dits and dahs and other timings
int rxDahLength = 0;
...
bool active = false;     // flag for trainer mode
bool startFirst = true;  // to indicate that we are starting a new sequence
                         // in the trainer modi
...
// for each character:
// byte length// byte morse encoding as binary value, beginning with most
// significant bit
...
const byte pool[][2] = {
    ...
    {B11100000, 5},  // 8
    {B11110000, 5},  // 9  35
    // interpunct   . , : - / = ? @ +    010101 110011 111000 100001 10010 10001
    // 001100 011010 01010
    {B01010100, 6},  // .  36
    {B11001100, 6},  // ,  37
    {B11100000, 6},  // :  38
...
bool keyTx = false;  // when state is set by manual key or touch paddle, then
                     // true! we use this to decide if Tx should be keyed or not
...
uint8_t
    loRaSerial;  /// a 6 bit serial number, start with some random value, will
                 /// be incremented witch each sent LoRa packet the first two
                 /// bits in teh byte will be the protocol id (CWLORAVERSION)
...
WebServer server(
    80);  // Create a webserver object that listens for HTTP request on port 80
...
// HTML for the AP server - ued to get SSID and Password for local WiFi network
// - needed for file upload and OTA SW updates
const char* myForm =
    "<html><head><meta charset='utf-8'><title>Get AP Info</title><style> form "
    "{width: 420px;}div { margin-bottom: 20px;}"
    "label {display: inline-block; width: 240px; text-align: right; "
    "padding-right: 10px;} button, input {float: right;}</style>"
    "</head><body>"
...
////// variables for Morse Decoder - the more global ones. rest is further
/// down...
////////////////////////////
/// variables for morse decoder
///////////////////////////////
uint32_t magnitudelimit;  // magnitudelimit_low = ( p_goertzelBandwidth? 80000 :
                          // 30000);
uint32_t magnitudelimit_low;  // magnitudelimit = magnitudelimit_low;

bool speedChanged = true;
bool filteredState = false;
bool filteredStateBefore = false;
...
int goertzel_n =
    152;  //// you can use:         152, 304, 456 or 608 - thats the max buffer
          /// reserved in checktone()
...
////////////////////////////////////////////////////////////////////
// encoder subroutines
/// interrupt service routine - needs to be positioned BEFORE all other
/// functions, including setup() and loop() interrupt service routine

void IRAM_ATTR isr() {  // Interrupt service routine is executed when a HIGH to
                        // LOW transition is detected on CLK
  ...
  delayMicroseconds(125);  // this seems to improve the responsiveness of the
                           // encoder and avoid any bouncing
...
bool adjustKeyerPreference(prefPos pos);
void audioLevelAdjust();
int16_t batteryVoltage();
void changeSpeed(int t);
int IRAM_ATTR checkEncoder();
bool checkPaddles();
void checkShutDown(bool enforce);
bool checkTone();
...
void displayScrollBar(bool visible);
...
void dispLoraLogo();
void doDecode();
bool doPaddleIambic(bool dit, bool dah);
void drawInputStatus(bool on);
void drawVolumeCtrl(bool inverse, uint16_t x, uint16_t y, uint16_t width,
                    uint16_t height, uint8_t volume);
void echoTrainerEval();
String encodeProSigns(String& input);
bool errorConnect(String msg);
void fetchNewWord();
void generateCW();
...
void initSensors();
void IRAM_ATTR isr();
void keyOut(bool on, bool fromHere, int f, int volume);
void keyTransmitter();
void loop();
...
uint8_t printOnScroll(uint8_t line, FONT_ATTRIB how, uint8_t xpos,
                      String mystring);
void printOnStatusLine(bool strong, uint8_t xpos, String string);
void printToScroll(FONT_ATTRIB style, String text);
void pwmClick(unsigned int volume);
void pwmNoTone();
void pwmTone(unsigned int frequency, unsigned int volume, bool lineOut);
...
void setup();
void shutMeDown();
void skipWords(File fn, uint32_t count);
void startAP();
void startMDNS();
...
void togglePolarity();
void updateFirmware();
void updatePaddleLatch(bool dit, bool dah);
...
// enable Vext
  ...
  // set up the encoder - we need external pull-ups as the pins used do not have
  // built-in pull-ups!
  pinMode(PinCLK, INPUT_PULLUP);
  pinMode(PinDT, INPUT_PULLUP);
  pinMode(keyerPin, OUTPUT);  // we can use the built-in LED to show when the
  ...
  esp_sleep_enable_ext0_wakeup(
      GPIO_NUM_0, (esp_sleep_ext1_wakeup_mode_t)0);  // 1 = High, 0 = Low
...
// we MUST reset the OLED RST pin for 50 ms! for the oldboard only
  ...
  display.init();
  display.flipScreenVertically();  // rotate 180°  when USB is to the left of
                                   // the module
  ...
  ledcAttachPin(lineOutPin, lineOutChannel);  ////// change this for real
                                              /// version - no line out currntly
  ...
  pinMode(volButtonPin, INPUT_PULLUP);  // external pullup for all GPIOS > 32
                                        // with ESP32-LORA wake up also works
                                        // without external pullup! Interesting!
  ...
  //// populate the array for abbreviations and words according to length and
  /// Koch filter
  ...
  // if version cannot be read, we have a new ESP32 and need to write the
  // preferences first

  pref.begin("morserino", false);  // open namespace "morserino" read-only

  /// new code for reading preoferences values - we check if we have a value,
  /// and if yes, we use it; if no, we use and write a default value
  ...
  if ((temp = pref.getUChar("lastExecuted"))) p_menuPtr = temp;
...
#define FORMAT_SPIFFS_IF_FAILED true
  ...
  if (!SPIFFS.begin(
          FORMAT_SPIFFS_IF_FAILED)) {  ///// if SPIFFS cannot be mounted, it
                                       /// does not exist. So create  (format)
                                       /// it, and mount it
  ...
  const char* defaultFile =
      "This is just an initial dummy file for the player. Dies ist nur die "
      "anfänglich enthaltene Standarddatei für den Player.\n"
      "Did you not upload your own file? Hast du keine eigene Datei "
      "hochgeladen?";

  if (!SPIFFS.exists("/player.txt")) {  // file does not exist, therefor we
                                        // create it from the text above
...
void displayStartUp() {
  ...
  if (volt < 3150)
    displayEmptyBattery();  // warn about empty battery and go to deep sleep
                            // (again)
...
void loop() {
  ...
  switch (morseState) {
    ...
    case morseTrx:
      ...
      break;
    case morseGenerator:
      if (leftKey || rightKey) {  // touching a paddle starts and stops the
                                  // generation of code
        // for debouncing:
        while (checkPaddles())
        ...
        if (!active) {
          // digitalWrite(keyerPin, LOW);           // turn the LED off, unkey
          // transmitter, or whatever pwmNoTone();
          keyOut(false, true, 0, 0);
        } else {
          generatorState = KEY_UP;
          genTimer =
              millis() - 1;  // we will be at end of KEY_DOWN when called the
                             // first time, so we can fetch a new word etc...
        }
      }  /// end squeeze
      if (active) generateCW();
      break;
    case echoTrainer:
      if (!active &&
          (leftKey || rightKey)) {  // touching both paddles starts and stops
        ...
        if (!active) {
          ...
          interWordTimer =
              4294967000;  // almost the biggest possible unsigned long number
                           // :-) - do not output a space at the beginning
        }
      }  /// end touch to start
      if (active) switch (echoTrainerState) {
          case START_ECHO:
          case SEND_WORD:
          case REPEAT_WORD:
            echoResponse = "";
  ...
  switch (volButton.clicks) {
    case 1:
      if (encoderState == scrollMode) {
      ...
      } else if (encoderState == volumeSettingMode &&
                 morseState != morseDecoder) {  //  single click toggles encoder
        ...
        if (pref.getUChar("sidetoneVolume") != p_sidetoneVolume)
          pref.putUChar(
              "sidetoneVolume",
              p_sidetoneVolume);  // store the last volume, if it has changed
        pref.end();
        displayCWspeed();
        displayVolume();
      } else {
    ...
    case -1:
      if (encoderState == scrollMode) {
        encoderState =
            (morseState == morseDecoder ? volumeSettingMode : speedSettingMode);
        relPos = maxPos;
        refreshScrollArea((bottomLine + 1 + relPos) % NoOfLines);
        displayScrollBar(false);
      } else {
  ...
  switch (modeButton.clicks) {  // actions based on enocder button
    case -1:
      menu_();  // long click exits current mode and goes to top menu
      return;
    case 1:
      if (morseState == morseGenerator ||
          morseState == echoTrainer) {  //  start/stop in trainer modi, in
                                        //  others does nothing currently
        active = !active;
        if (!active) {
          // digitalWrite(keyerPin, LOW);           // turn the LED off, unkey
          // transmitter, or whatever pwmNoTone();
      ...
      break;
    case 2:
      setupPreferences();  // double click shows the preferences menu
...
void menu_() {
  ...
  keyerState = IDLE_STATE;
  interWordTimer =
      4294967000;  // almost the biggest possible unsigned long number :-) - do
                   // not output a space at the beginning
  genTimer = millis() - 1;     // we will be at end of KEY_DOWN when called the
                               // first time, so we can fetch a new word etc...
  printToScroll(REGULAR, "");  // clear the buffer
  // pwmNoTone();                               // stop side tone  - just in
  // case digitalWrite(keyerPin, LOW);               // turn the LED off, unkey
  // transmitter, or whatever; just in case....
  keyOut(false, true, 0, 0);
  keyOut(false, false, 0, 0);
  encoderState = speedSettingMode;  // always start with this encoderstate
                                    // (decoder will change it anyway)
  currentOptions =
      allOptions;  // this is the array of options when we double click the
                   // BLACK button: while in menu, you can change all of them
  currentOptionSize = SizeOfArray(allOptions);
  pref.begin("morserino", false);  // open the namespace as read/write
  if ((p_fileWordPointer !=
       pref.getUInt("fileWordPtr")))  // update word pointer if necessary (if we
                                      // ran player before)
  ...
  while (true) {  // we wait for a click (= selection)
    ...
    switch (modeButton.clicks) {  // actions based on enocder button
      case 2:
        setupPreferences();  // all available options when called from top menu
        menuDisplay(newMenuPtr);
        break;
      case 1:  // check if we have a submenu or if we execute the selection
        // Serial.println("newMP: " + String(newMenuPtr) + " navi: " +
        // String(menuNav[newMenuPtr][naviDown]));
        if (menuNav[newMenuPtr][naviDown] == 0) {
          p_menuPtr = newMenuPtr;
          disp = 0;
          if (p_menuPtr !=
              _goToSleep) {  // remember last exectued, unless it is shutdown
            pref.begin("morserino", false);  // open the namespace as read/write
            pref.putUChar("lastExecuted",
                          p_menuPtr);  // store last executed command
    ...
    switch (volButton.clicks) {
      case -1:
        audioLevelAdjust();  /// for adjusting line-in audio level (at the same
                             /// time keying tx and sending oudio on line-out
        display.clear();
        menuDisplay(disp);
        break;
      case 3:
        wifiFunction();  /// configure wifi, upload file or firmware update
        break;
    }
    checkShutDown(false);  // check for time out
...
void menuDisplay(uint8_t ptr) {
  // Serial.println("Level: " + (String) menuNav [ptr][naviLevel] + " " +
  // menuText[ptr]);
  ...
  clearLine(0);
  clearLine(1);
  clearLine(2);  // delete previous content
  ...
  switch (menuNav[ptr][naviLevel]) {
    case 2:
      printOnScroll(2, BOLD, 0, menuText[ptr]);
      printOnScroll(1, REGULAR, 0, menuText[oneUp]);
      printOnScroll(0, REGULAR, 0, menuText[twoUp]);
      break;
    case 1:
      if (oneDown) printOnScroll(2, REGULAR, 0, String(".."));
      printOnScroll(1, BOLD, 0, menuText[ptr]);
      printOnScroll(0, REGULAR, 0, menuText[oneUp]);
      break;
    case 0:
      if (oneDown) printOnScroll(1, REGULAR, 0, String(".."));
      printOnScroll(0, BOLD, 0, menuText[ptr]);
      break;
  }
}

///////////// eGEN_TYPE { RANDOMS, ABBREVS, WORDS, CALLS, MIXED, KOCH_MIXED,
/// KOCH_LEARN };           // the things we can generate in generator mode

bool menuExec() {  // return true if we should  leave menu after execution,
                   // true if we should stay in menu
  ...
  switch (p_menuPtr) {
    ...
    case _genMixed:  /// generator
      generatorMode =
          (GEN_TYPE)(p_menuPtr - 3);  /// 0 = RANDOMS ... 4 = MIXED, 5 = PLAYER
      currentOptions =
          generatorOptions;  // list of available options in generator mode
    ...
    case _genPlayer:
      generatorMode =
          (GEN_TYPE)(p_menuPtr - 3);  /// 0 = RANDOMS ... 4 = MIXED, 5 = PLAYER
      currentOptions =
          playerOptions;  // list of available options in player mode
    ...
    case _echoMixed:
      currentOptions =
          echoTrainerOptions;  // list of available options in echo trainer mode
      currentOptionSize = SizeOfArray(echoTrainerOptions);
      generatorMode =
          (GEN_TYPE)(p_menuPtr - 10);  /// 0 = RANDOMS ... 4 = MIXED, 5 = PLAYER
      goto startEcho;
    case _echoPlayer:  /// echo trainer
      generatorMode =
          (GEN_TYPE)(p_menuPtr - 10);  /// 0 = RANDOMS ... 4 = MIXED, 5 = PLAYER
      currentOptions =
          echoPlayerOptions;  // list of available options in echo player mode
    ...
    startEcho:
      ...
      printOnScroll(
          0, REGULAR, 0,
          generatorMode == KOCH_LEARN ? "New Character:" : "Echo Trainer:");
    ...
    case _kochLearn:  // Koch Learn New .  /// just a new generatormode....
      ...
      currentOptions =
          kochEchoOptions;  // list of available options in Koch echo mode
    ...
    case _kochGenRand:  // RANDOMS
      ...
      kochActive = true;
      currentOptions =
          kochGenOptions;  // list of available options in Koch generator mode
    ...
    case _kochEchoRand:  // Koch Echo Random
      generatorMode = RANDOMS;
      kochActive = true;
      currentOptions = kochEchoOptions;  // list of available options in Koch
                                         // echo trainer mode
      currentOptionSize = SizeOfArray(kochEchoOptions);
      goto startEcho;
    ...
    case _trxLora:  // LoRa Transceiver
      currentOptions =
          loraTrxOptions;  // list of available options in lora trx mode
    ...
    case _trxIcw:  /// icw/ext TRX
      currentOptions =
          extTrxOptions;  // list of available options in ext trx mode
    ...
    case _decode:  /// decoder
      currentOptions =
          decoderOptions;  // list of available options in lora trx mode
...
bool doPaddleIambic(bool dit, bool dah) {
  bool paddleSwap;          // temp storage if we need to swap left and right
  ...
  switch (keyerState) {  // this is the keyer state machine
    case IDLE_STATE:
      ...
      if (millis() > interWordTimer) {
        ...
        if (echoTrainerState == COMPLETE_ANSWER) {  // change the state of the
      ...
      if (dit || dah) {
        ...
        if (morseState ==
            echoTrainer) {  // change the state of the trainer at end of word
        ...
        return false;  // we return false if there was no paddle press in IDLE
                       // STATE - Arduino can do other tasks for a bit
      }
      break;
    ...
    case DIT:
      ...
      if (p_ACSlength > 0 &&
          (millis() <= acsTimer))  // if we do automatic character spacing, and
                                   // haven't waited for (3 or whatever) dits...
        break;
      clearPaddleLatches();  // always clear the paddle latches at beginning of
                             // new element
      ...
      switch (p_keyermode) {
        case IAMBICB:
          curtistimer = ditLength * p_curtisBDotTiming / 100;
          break;  // enhanced Curtis mode B starts checking after some time
        case IAMBICA:
        case ULTIMATIC:
          curtistimer = ditLength;  // no early paddle checking in Curtis mode A
                                    // or Ultimatic mode
          break;
      }
    ...
    case DAH:
      if (p_ACSlength > 0 &&
          (millis() <= acsTimer))  // if we do automatic character spacing, and
                                   // haven't waited for 3 dits...
        break;
      clearPaddleLatches();  // clear the paddle latches
      keyerControl &=
          ~(DIT_LAST);  // clear dit latch  - we are not processing a DIT

      ktimer = dahLength;
      switch (p_keyermode) {
        case IAMBICB:
          curtistimer =
              dahLength * p_curtisBTiming /
              100;  // enhanced Curtis mode B starts checking after some time
          break;
        case IAMBICA:
        case ULTIMATIC:
          curtistimer = dahLength;  // no early paddle checking in Curtis mode A
                                    // or Ultimatic mode
          break;
      }
      keyerState = KEY_START;  // set next state of state machine
    ...
    case KEYED:
      ...
      if (millis() > ktimer) {  // are we at end of key down ?
        // digitalWrite(keyerPin, LOW);        // turn the LED off, unkey
        // transmitter, or whatever pwmNoTone();                      // stop
        // side tone
        keyOut(false, true, 0, 0);
        ktimer = millis() + ditLength;      // inter-element time
        keyerState = INTER_ELEMENT;         // next state
      } else if (millis() > curtistimer) {  // in Curtis mode we check paddle as
                                            // soon as Curtis time is off
        if (keyerControl & DIT_LAST)        // last element was a dit
          updatePaddleLatch(false,
                            dah);  // not sure here: we only check the opposite
                                   // paddle - should be ok for Curtis B
        else
          updatePaddleLatch(dit, false);
        // updatePaddleLatch(dit, dah);       // but we remain in the same state
        // until element time is off!
      }
      break;
        ...
        switch (keyerControl) {
          ...
          case 7:
            if (p_keyermode != ULTIMATIC)  // we are in a Iambic mode
              if (keyerControl &
                  DIT_LAST)  // last element was a dit - this is case 7, really
          ...
          case 0:
          case 4:
            keyerState = IDLE_STATE;  // we are at the end of the character and
                                      // go back into IDLE STATE
            displayMorse();           // display the decoded morse character(s)
            if (morseState == loraTrx) cwForLora(0);

            ++charCounter;  // count this character
            // if we have seen 12 chars since changing speed, we write the
            // config to preferences (speed and left & right thresholds)
            if (charCounter == 12) {
              pref.begin("morserino",
                         false);  // open the namespace as read/write
              pref.putUChar("wpm", p_wpm);
            ...
            if (p_ACSlength > 0)
              acsTimer =
                  millis() + p_ACSlength * ditLength;  // prime the ACS timer
            if (morseState == morseKeyer || morseState == loraTrx ||
                morseState == morseTrx)
              interWordTimer = millis() + 6 * ditLength;
            else
              interWordTimer =
                  millis() +
                  interWordSpace;  // prime the timer to detect a space between
                                   // characters nominally 7 dit-lengths, but we
                                   // are not quite so strict here in keyer or
                                   // TrX mode, use the extended time in echo
                                   // trainer mode to allow longer space between
                                   // characters, like in listening
            keyerControl =
                0;  // clear all latches completely before we go to IDLE
            break;
...
//// this function checks the paddles (touch or external), returns true when a
/// paddle has been activated,
///// and sets the global variable leftKey and rightKey accordingly

bool checkPaddles() {
  static bool oldL = false, newL, oldR = false, newR;
  ...
  if (newL != oldL) lTimer = millis();
  if (newR != oldR) rTimer = millis();
  if (millis() - lTimer > debDelay)
    if (newL != leftKey) leftKey = newL;
  if (millis() - rTimer > debDelay)
    if (newR != rightKey) rightKey = newR;
...
void setDITstate() {
  ...
  if (morseState == loraTrx) cwForLora(1);  // build compressed string for LoRA
}

void setDAHstate() {
  keyerState = DAH;
  treeptr = CWtree[treeptr].dah;
  if (morseState == loraTrx) cwForLora(2);
}
...
void displayMorse() {
  ...
  if (morseState ==
      echoTrainer) {  /// store the character in the response string
    symbol.replace("<as>", "S");
    symbol.replace("<ka>", "A");
    symbol.replace("<kn>", "N");
    symbol.replace("<sk>", "K");
...
void pwmTone(unsigned int frequency, unsigned int volume,
             bool lineOut) {  // frequency in Hertz, volume in range 0 - 100;
                              // we use 10 bit resolution
...
void pwmNoTone() {  // stop playing a tone by changing duty cycle of the tone to
                    // 0
  ledcWrite(toneChannel, dutyCycleZero);
...
void displayTopLine() {
  ...
  if (morseState == morseGenerator)
    ...
    switch (p_keyermode) {
      case IAMBICA:
        printOnStatusLine(false, 2, "A ");
        break;  // 0 means we are Iambic A (no paddle eval during dah)
      case IAMBICB:
        printOnStatusLine(false, 2, "B ");
        break;  // 1: orig Curtis mode: paddle eval during element
      case ULTIMATIC:
        printOnStatusLine(false, 2, "U ");
        break;  // 2: Ultimatic Mode
    }
  }
  displayCWspeed();  // update display of CW speed
  if ((morseState == loraTrx) ||
      (morseState == morseGenerator && p_loraTrainerMode == true))
    dispLoraLogo();
  displayVolume();  // sidetone volume
  display.display();
}

void dispLoraLogo() {  // display a small logo in the top right corner to
                       // indicate we operate with LoRa
...
void displayCWspeed() {
  ...
  printOnStatusLine(encoderState == speedSettingMode ? true : false, 7,
                    numBuffer);
...
uint8_t readSensors(int left, int right) {
  ...
  if (lValue < (p_tLeft + 10)) {  // adaptive calibration
    ...
    // printOnScroll(0, INVERSE_BOLD, 0,  String(lValue) + " " +
    // String(p_tLeft));
  ...
  if (rValue < (p_tRight + 10)) {  // adaptive calibration
    ...
    // printOnScroll(1, INVERSE_BOLD, 0,  String(rValue) + " " +
    // String(p_tRight));
...
String getRandomWord(
    int maxLength) {  //// give me a random English word, max maxLength chars
                      /// long (1-5) - 0 returns any length
...
String getRandomAbbrev(
    int maxLength) {  //// give me a random CW abbreviation , max maxLength
                      /// chars long (1-5) - 0 returns any length
  if (maxLength > 5) maxLength = 0;
  if (kochActive)
    return kochAbbr[random(numberOfAbbr)];
  else
    return abbreviations[random(ABBREV_POINTER[maxLength],
                                ABBREV_NUMBER_OF_ELEMENTS)];
}
...
//  {OPT_ALL, OPT_ALPHA, OPT_NUM, OPT_PUNCT, OPT_PRO, OPT_ALNUM, OPT_NUMPUNCT,
//  OPT_PUNCTPRO, OPT_ALNUMPUNCT, OPT_NUMPUNCTPRO}

String getRandomChars(
    int maxLength, int option) {  /// random char string, eg. group of 5, 9
                                  /// differing character pools; maxLength = 1-6
  String result = "";
  String pool;
  int s = 0, e = 50;
  int i;
  if (maxLength > 6) {  // we use a random length!
    maxLength =
        random(2, maxLength - 3);  // maxLength is max 10, so random upper limit
                                   // is 7, means max 6 chars...
  }
  if (kochActive) {  // kochChars =
                     // "mkrsuaptlowi.njef0yv,g5/q9zh38b?427c1d6x-=KA+SNE@:"
    int endk = p_kochFilter;  //              1   5    1    5    2    5    3 5
                              //              4    5    5
    for (i = 0; i < maxLength; ++i) {
      if (random(2))  // in Koch mode, we generate the last third of the chars
                      // learned  a bit more often
    ...
    for (i = 0; i < maxLength; ++i) result += CWchars.charAt(random(s, e));
  }
  return result;
}

String getRandomCall(
    int maxLength) {  // random call-sign like pattern, maxLength = 3 - 6, 0
                      // returns any length
  const byte prefixType[] = {1, 0, 1,
                             2, 3, 1};  // 0 = a, 1 = aa, 2 = a9, 3 = 9a
  byte prefix;
  String call = "";
  unsigned int l = 0;
  // int s, e;

  if (maxLength == 1 || maxLength == 2) maxLength = 3;
  ...
  call += CWchars.charAt(random(26, 36));
  ++l;
  // generate a suffix, 1 2 or 3 chars long - we re-use prefix for the suffix
  // length
  if (maxLength == 3)
    prefix = 1;
  else if (maxLength == 0) {
    ...
    prefix =
        (prefix == 2
             ? prefix
             : random(1,
                      4));  // increase the likelihood for suffixes of length 2
  } else {
    // prefix = random(1,_min(maxLength-l+1, 4));     // suffix not longer than
    // 3 chars!
    prefix =
        _min(maxLength - l, 3);  // we try to always give the desired length,
                                 // but never more than 3 suffix chars
  }
  while (prefix--) {
    call += CWchars.charAt(random(0, 26));
    ++l;
  }  // now we have the suffix as well
  // are we /p or /m? - we do this only in rare cases - 1 out of 9, and only
  // when maxLength = 0, or maxLength-l >= 2
...
String generateCWword(String symbols) {
  ...
  for (int i = 0; i < l; ++i) {
    ...
    pointer =
        CWchars.indexOf(c);  // at which position is the character in CWchars?
    NoE = pool[pointer][1];  // how many elements in this morse code symbol?
    bitMask = pool[pointer][0];  // bitMask indicates which of the elements are
                                 // dots and which are dashes
    for (int j = 0; j < NoE; ++j) {
      result += (bitMask & B10000000
                     ? "2"
                     : "1");  // get MSB and store it in string - 2 is dah, 1 is
                              // dit, 0 = inter-element space
      bitMask = bitMask << 1;  // shift bitmask 1 bit to the left
      // Serial.print("Bitmask: ");
      // Serial.println(bitmask, BIN);
    }  /// now we are at the end of one character, therefore we add enough space
       /// for inter-character
    result += "0";
...
void generateCW() {  // this is called from loop() (frequently!)  and generates
                     // CW
  ...
  switch (generatorState) {  // CW generator state machine - key is up or down
    case KEY_UP:
      ...
      if (startFirst == true) CWword = "";
      l = CWword.length();

      if (l == 0) {  // fetch a new word if we have an empty word
        if (clearText.length() > 0) {  // this should not be reached at all....
        ...
        if (CWword.length() == 0)  // we really should have something here -
                                   // unless in trx mode; in this case return
          return;
      }
      c = CWword[0];  // retrieve next element from CWword; if 0, we were at end
                      // of character
      CWword.remove(0, 1);
      ...
      if (morseState != loraTrx)
        genTimer =
            millis() +
            (c == '1' ? ditLength : dahLength);  // start a dit or a dah,
                                                 // acording to the next element
      else
        genTimer = millis() + (c == '1' ? rxDitLength : rxDahLength);
      if (morseState == morseGenerator &&
          p_loraTrainerMode == 1)  // send the element to LoRa
      ...
      keyOut(true, (morseState != loraTrx), notes[p_sidetoneFreq],
             p_sidetoneVolume);
      ...
      break;
    case KEY_DOWN:
      if (millis() < genTimer)  // if not at end of key down we need to wait, so
                                // we just return to loop()
        return;
      //// otherwise we continue here; stop keying,  and determine the length of
      /// the following pause: inter Element, interCharacter or InterWord?
      // pwmNoTone();                                            // stop side
      // tone digitalWrite(keyerPin, LOW);                            // if we
      // key in trainer mode, or not....
      keyOut(false, (morseState != loraTrx), 0, 0);
      if (!CWword.length()) {  // we just ended the the word, ...  ////
                               // intercept here in Echo Trainer mode
        ...
        if (morseState == echoTrainer) {
          switch (echoTrainerState) {
            case START_ECHO:
              ...
              genTimer = millis() + interCharacterSpace +
                         (p_promptPause * interWordSpace);
              break;
            case REPEAT_WORD:
              // fall through
            case SEND_WORD:
          ...
          genTimer =
              ...
              (morseState == loraTrx
                   ? rxInterWordSpace
                   : interWordSpace);  // we need a pause for interWordSpace
          if (morseState == morseGenerator &&
              p_loraTrainerMode ==
                  1) {  // in generator mode and we want to send with LoRa
            cwForLora(0);
            cwForLora(3);  // as we have just finished a word
            // Serial.println("cwForLora(3);");
            sendWithLora();  // finalise the string and send it to LoRA
            delay(interCharacterSpace +
                  ditLength);  // we need a slightly longer pause otherwise the
                               // receiving end might fall too far behind...
          }
        }
        ...
        genTimer = millis() +
                   (morseState == loraTrx
                        ? rxInterCharacterSpace
                        : interCharacterSpace);  // pause = intercharacter space
      } else {  // we are in the middle of a character
        genTimer = millis() + (morseState == loraTrx
                                   ? rxDitLength
                                   : ditLength);  // pause = interelement space
      }
...
void dispGeneratedChar() {
  ...
  if (generatorMode == KOCH_LEARN || morseState == loraTrx ||
      (p_trainerDisplay == DISPLAY_BY_CHAR && echoTrainerState != SEND_WORD &&
       echoTrainerState != REPEAT_WORD))

  {  /// we need to output the character on the display now
    charString =
        clearText.charAt(0);  /// store first char of clearText in charString
    clearText.remove(0, 1);   /// and remove it from clearText
    if (generatorMode == KOCH_LEARN)
      printToScroll(REGULAR, "");  // clear the buffer first
    printToScroll(morseState == loraTrx ? BOLD : REGULAR,
  ...
  // if we have seen 12 chars since changing speed, we write the config to
  // Preferences
  if (charCounter == 12) {
    pref.begin("morserino", false);  // open the namespace as read/write
    pref.putUChar("wpm", p_wpm);
...
void fetchNewWord() {
  int rssi, rxWpm, rv;
  static bool firstTime = true;  /// for word doubler mode

  if (morseState ==
      loraTrx) {      // we check the rxBuffer and see if we received something
    updateSMeter(0);  // at end of word we set S-meter to 0 until we receive
                      // something again
    ...
    if (loRaBuReady()) {
      ...
      rxDitLength =
          1200 /
          rxWpm;  // set new value for length of dits and dahs and other timings
    ...
    if (morseState != echoTrainer)
      printToScroll(
          REGULAR,
          " ");  /// in any case, add a blank after the word on the display
    if (generatorMode == KOCH_LEARN) {
      startFirst = false;
      echoTrainerState = SEND_WORD;
    }
    if (startFirst == true) {  /// do the intial sequence in trainer mode, too
      clearText = "vvvA";
      startFirst = false;
    } else if (morseState == morseGenerator && p_wordDoubler == true &&
               firstTime == false) {
      clearText = echoTrainerWord;
      firstTime = true;
    } else if (morseState == echoTrainer) {
      interWordTimer =
          4294967000;  /// interword timer should not trigger something now
      // Serial.println("echoTrainerState: " + String(echoTrainerState));
      switch (echoTrainerState) {
        case REPEAT_WORD:
          if (p_echoRepeats == 7 || repeats <= p_echoRepeats)
            clearText = echoTrainerWord;
          else {
            clearText = echoTrainerWord;
            if (generatorMode != KOCH_LEARN) {
              printToScroll(INVERSE_REGULAR,
                            cleanUpProSigns(clearText));  //// clean up first!
    ...
    randomGenerate:
      ...
      switch (generatorMode) {
        case RANDOMS:
          clearText = getRandomChars(p_randomLength, p_randomOption);
          break;
        case CALLS:
          clearText = getRandomCall(p_callLength);
          break;
        case ABBREVS:
          clearText = getRandomAbbrev(p_abbrevLength);
          break;
        case WORDS:
          clearText = getRandomWord(p_wordLength);
          break;
        case KOCH_LEARN:
          clearText = (String)kochChars.charAt(p_kochFilter - 1);
          break;
        case MIXED:
          rv = random(4);
          switch (rv) {
            ...
            case 2:
              ...
              break;
            case 3:
              clearText = getRandomChars(
                  1, OPT_PUNCTPRO);  // just a single pro-sign or interpunct
              break;
          }
          break;
        case KOCH_MIXED:
          ...
          switch (rv) {
            ...
            case 2:
              clearText =
                  getRandomChars(p_randomLength, OPT_KOCH);  // Koch option!
              break;
          }
          break;
        case PLAYER:
          clearText = getWord(file);
          if (clearText ==
              String()) {  /// at end of file: go to beginning again
            p_fileWordPointer = 0;
            file.close();
      ...
      firstTime = false;
    }  /// end if else - we either already had something in trainer mode, or we
       /// got a new word
...
void updateSMeter(int rssi) {
  ...
  if (rssi == 0)
    ...
    drawVolumeCtrl(false, 93, 0, 28, 15,
                   constrain(map(rssi, -150, -20, 0, 100), 0, 100));
    wasZero = false;
  }
  display.display();
}
...
void setupPreferences() {
  // enum morserinoMode {morseKeyer, loraTrx, morseGenerator, echoTrainer,
  // shutDown, morseDecoder, invalid };
  static int oldPos = 1;
  int t;
  ...
  ///// we should check here if the old ptr (oldIndex) is contained in the
  /// current preferences collection (currentOptions)
  ptrIndex = 1;
  ...
  while (true) {  // we wait for single click = selection or long click = exit
    ...
    switch (modeButton.clicks) {  // button was clicked
      case 1:                     // change the option corresponding to pos
        if (adjustKeyerPreference(posPtr)) goto exitFromHere;
        break;
      case -1:  //////// long press indicates we are done with setting
                /// preferences - check if we need to store some of the
                /// preferences
      exitFromHere:
        updatePreferences();
        // delay(200);
...
void displayCurtisMode() {
  String keyerModus[] = {"Curtis A    ", "Curtis B    ", "Ultimatic   "};
  printOnScroll(2, REGULAR, 1, keyerModus[p_keyermode - 1]);
}

void displayCurtisBTiming() {
  // display start timing when paddles are being checked in Curtis B mode during
  // dah: between 0 and 100
  sprintf(numBuffer, "%3i", p_curtisBTiming);
  printOnScroll(2, REGULAR, 1, numBuffer);
}

void displayCurtisBDotTiming() {
  // display start timing when paddles are being checked in Curtis B modeduring
  // dit : between 0 and 100
  sprintf(numBuffer, "%3i", p_curtisBDotTiming);
  printOnScroll(2, REGULAR, 1, numBuffer);
}

void displayACS() {
  String ACSmode[] = {"Off         ", "Invalid     ", "min. 2 dots ",
                      "min. 3 dots ", "min. 4 dots "};
...
void displayExtPaddles() {
  printOnScroll(2, REGULAR, 1,
                p_useExtPaddle ? "Ext. Paddle " : "Touch Paddle");
}
...
void displayRandomOption() {
  String texts[] = {"All Chars   ", "Alpha       ", "Numerals    ",
                    "Interpunct. ", "Pro Signs   ", "Alpha + Num ",
                    "Num+Interp. ", "Interp+ProSn", "Alph+Num+Int",
                    "Num+Int+ProS"};
...
void displayKochFilter() {  // const String kochChars =
                            // "mkrsuaptlowi.njef0yv,g5/q9zh38b?427c1d6x-=KA+SNE@:";
  String str;
  str.reserve(6);
...
bool adjustKeyerPreference(
    prefPos pos) {  /// rotating the encoder changes the value, click returns to
                    /// preferences menu
  // printOnScroll(1, REGULAR, 0, " ");       /// returns true when a long
  // button press ended it, and false when there was a short click
  ...
  while (true) {  // we wait for single click = selection or long click = exit
    ...
    if ((t = checkEncoder())) {
      ...
      switch (pos) {
        ...
        case posCurtisBDahTiming:
          p_curtisBTiming +=
              (t * 5);  // Curtis B timing dah (enhanced Curtis mode)
          p_curtisBTiming = constrain(p_curtisBTiming, 0, 100);
          displayCurtisBTiming();
          break;
        case posCurtisBDotTiming:
          p_curtisBDotTiming +=
              (t * 5);  // Curtis B timing dit (enhanced Curtis mode)
          p_curtisBDotTiming = constrain(p_curtisBDotTiming, 0, 100);
          displayCurtisBDotTiming();
          break;
        case posACS:
          p_ACSlength += (t + 1);  // ACS
          if (p_ACSlength == 2) p_ACSlength += t;
        ...
        case posKeyTrainerMode:
          p_keyTrainerMode +=
              (t + 1);  // Key TRX: 0=never, 1= keyer only, 2 = keyer & trainer
          p_keyTrainerMode = constrain(p_keyTrainerMode - 1, 0, 2);
          displayKeyTrainerMode();
          break;
        case posInterWordSpace:
          p_interWordSpace += t;  // interword space in lengths of dit
          p_interWordSpace = constrain(p_interWordSpace, 6,
                                       45);  // has to be between 6 and 45 dits
          displayInterWordSpace();
          updateTimings();
          break;
        case posInterCharSpace:
          p_interCharSpace =
              constrain(p_interCharSpace + t, 3,
                        15);  // set Interchar space - 3 - 15 dits
          displayInterCharSpace();
          updateTimings();
          break;
        case posKochFilter:
          ...
          displayKochFilter();
          break;
        // case  posGenerate : p_generatorMode = (p_generatorMode + t + 6) % 6;
        // // what trainer generates (0 - 5)
        //               displayGenerate();
        //               break;
        case posRandomOption:
          p_randomOption = (p_randomOption + t + 10) %
                           10;  // which char set for random chars?
          displayRandomOption();
          break;
        case posRandomLength:
          p_randomLength += t;  // length of random char group: 2-6
          p_randomLength =
              constrain(p_randomLength, 1, 10);  // 7-10 for rnd length 2 to 3-6
          displayRandomLength();
          break;
        case posCallLength:
        ...
        case posWordLength:
          ...
          break;
        case posTrainerDisplay:
          p_trainerDisplay = (p_trainerDisplay + t + 3) %
                             3;  // display options for trainer: 0-2
          displayTrainerDisplay();
          break;
        case posEchoRepeats:
        ...
        case posEchoToneShift:
          p_echoToneShift +=
              (t + 1);  // echo tone shift can be 0, 1 (up) or 2 (down)
        ...
        case posLoraTrainerMode:
          p_loraTrainerMode +=
              (t + 2);  // transmit lora in generator and player mode; can be 0
                        // (no) or 1 (yes)
...
void updateTimings() {
  ditLength =
      1200 /
      p_wpm;  // set new value for length of dits and dahs and other timings
  dahLength = 3 * ditLength;
  interCharacterSpace = p_interCharSpace * ditLength;
  interWordSpace =
      _max(p_interWordSpace * ditLength, (p_interCharSpace + 4) * ditLength);
  effWpm =
      60000 / (31 * ditLength + 4 * interCharacterSpace +
               interWordSpace);  /// we do not use it yet! effective wpm with
                                 /// lengthened spaces = Farnsworth speed
}
...
void keyTransmitter() {
  if (p_keyTrainerMode == 0 || morseState == echoTrainer ||
      morseState == loraTrx)
  ...
  if (keyTx == false) return;  // do not key when input is from tone decoder
  digitalWrite(keyerPin,
               HIGH);  // turn the LED on, key transmitter, or whatever
}

void createKochWords(
    uint8_t maxl,
    uint8_t koch) {  // this function creates an array of words that are
                     // compliant to Koch filter and max word length
  numberOfWords = 0;
  for (int i = WORDS_POINTER[maxl]; i < WORDS_NUMBER_OF_ELEMENTS;
       ++i) {  // do this for all words with max length maxl
    if (wordIsKoch(words[i]) <= koch) kochWords[numberOfWords++] = words[i];
  }
}
...
void createKochAbbr(
    ...
    uint8_t koch) {  // this function creates an array of words that are
                     // compliant to Koch filter and max word length
  numberOfAbbr = 0;
  for (int i = ABBREV_POINTER[maxl]; i < ABBREV_NUMBER_OF_ELEMENTS;
       ++i) {  // do this for all words with max length maxl
...
void displayVolume() {
  drawVolumeCtrl(encoderState == speedSettingMode ? false : true, 93, 0, 28, 15,
                 p_sidetoneVolume);
  display.display();
}

///// functions to use the graphics display more or less like a character
/// display
///// basically we use two different fixed-width fonts, Dialoginput 12 (for the
/// status line) and Dialoginput 15 for almost everything else
...
//// printOnScroll(line, xpos, string); line (0,1 or 2), xpos 0 = leftmost
/// postition / clearTopLine(), clearMiddleLine(), clearBottomLine() /
/// printToScroll(char letter); push a character onto the scrolling display /
/// starting coordinates of the 4 lines: / status line: 0,0 / top line:    0,15
//// middle line: 0,31
//// bottom line: 0.47
...
void printOnStatusLine(
    bool strong, uint8_t xpos,
    String string) {  // place a string onto the status line; chars are 7px wide
                      // = 18 chars per line
  if (strong)
...
void printToScroll(FONT_ATTRIB style, String text) {
  /// store text in textBuffer, if it fits the screen line; otherwise scroll up,
  /// clear bottom buffer, store in new buffer, print on new lien
  ...
  if (screenPos + l >
      NoOfCharsPerLine) {  // we need to scroll up and start a new line
    newLine();
    pos = 0;
    screenPos = 0;
    lastStyle = REGULAR;
  ...
  if (style == REGULAR) {
    memcpy(&textBuffer[bottomLine][pos], text.c_str(),
           l);  // copy the string of characters
    pos += l;
    ...
    if (style == lastStyle) {  // not regular, but we have no change in style!
      ...
      textBuffer[bottomLine][pos++] =
          (char)style;  // add the style marker at the beginning
      memcpy(&textBuffer[bottomLine][pos], text.c_str(),
             l);  // copy the string of characters
      pos += l;
      textBuffer[bottomLine][pos++] =
          (char)style;                        // add the style marker at the end
      textBuffer[bottomLine][pos] = (char)0;  // add 0 character
      lastStyle = style;                      // remember new style flag
    }
  }

  if (relPos == maxPos) {  // we show the bottom lines on the screen, therefore
                           // we add the new stuff  immediately
    /// and send string to screen, avoiding refresh of complete line
    printOnScroll(2, style, screenPos,
                  text);  // these characters are 9 pixels wide,
  }
...
void newLine() {
  ...
  if (relPos && relPos != maxPos) --relPos;
  bottomLine = linePointer;
  textBuffer[bottomLine][0] = (char)0;  /// and empty the bottom line
  if (!relPos)  /// screen ptr already on top, we need to move the whole screen
                /// one line
    refreshScrollArea((bottomLine + 1) % NoOfLines);
  else if (relPos == maxPos)
    refreshScrollArea((NoOfLines + bottomLine - 2) % NoOfLines);
  if (encoderState == scrollMode) displayScrollBar(true);
}

void refreshScrollArea(int pos) {  /// refresh all three lines from buffer in
                                   /// scroll area; pos is topmost buffer line
...
void refreshScrollLine(int bufferLine,
                       int displayLine) {  /// print a line to the screen
  String temp;
  temp.reserve(16);
  temp = "";
  char c;
  ...
  display.setColor(BLACK);
  display.fillRect(0, SCROLL_TOP + displayLine * LINE_HEIGHT, 127,
  ...
  if (temp.length()) printOnScroll(displayLine, style, pos, temp);
}

uint8_t printOnScroll(
    uint8_t line, FONT_ATTRIB how, uint8_t xpos,
    String mystring) {  // place a string onto the scroll area; line = 0, 1 or 2
  uint8_t w;
  ...
  display.fillRect(xpos * 9, SCROLL_TOP + line * LINE_HEIGHT, w,
                   LINE_HEIGHT + 1);
  ...
  display.display();
  resetTOT();
  return w;  // we return the actual width of the output, in case of converted
             // UTF8 characters
}

void clearLine(
    uint8_t line) {  /// clear a line - display is done somewhere else!
  display.setColor(BLACK);
...
#if BOARDVERSION == 3
  WiFi.mode(WIFI_MODE_NULL);  // make sure WiFi is not running, as it uses the
                              // same ADC as battery measurement!
  const float XS = 1.43766;  // The returned reading is multiplied by this XS to
...
void displayEmptyBattery() {  /// display a warning and go to (return to) deep
                              /// sleep
...
void resetTOT() {  //// reset the Time Out Timer - we do this whenever there is
                   /// a screen update
  TOTcounter = millis();
}

void checkShutDown(bool enforce) {  /// if enforce == true, we shut donw even
                                    /// if there was no time-out
  long int timeOut = ((morseState == loraTrx) || (morseState == morseTrx))
                         ? 450000
                         : 300000;  /// 7,5 or 5 minutes
...
void updatePreferences() {
  ...
  if (p_sidetoneFreq != pref.getUChar("sidetoneFreq"))
    pref.putUChar("sidetoneFreq", p_sidetoneFreq);
  if (p_didah != pref.getBool("didah")) pref.putBool("didah", p_didah);
...
/// cwForLora packs element info (dit, dah, interelement space) into a String
/// that can be sent via LoRA
///  element can be:
///  0: inter-element space
///  1: dit
///  2: dah
///  3: end of word -: cwForLora returns a string that is ready for sending to
...
void cwForLora(int element) {
  ...
  if (pairCounter == 0) {  // we start a brand new word for LoRA - clear buffer
                           // and set speed first
    for (int i = 0; i < 32; ++i) loraTxBuffer[i] = (char)0;

    /// 1st byte: version + serial number
    header = ++loRaSerial % 64;
    // Serial.println("loRaSerial: " + String(loRaSerial));
    header += CWLORAVERSION *
              64;  //// shift VERSION left 6 bits and add to the serial number
    loraTxBuffer[0] = header;

    temp = p_wpm * 4;  /// shift left 2 bits
    loraTxBuffer[1] |= temp;
    pairCounter = 7;  /// so far we have used 7 bit pairs: 4 in the first byte
                      /// (protocol version+serial); 3 in the 2nd byte (wpm)
    // Serial.println(temp);
    // Serial.println(loraBuffer);
  }

  temp =
      element & B011;  /// take the two left bits
                       // Serial.println("Temp before shift: " + String(temp));
  /// now store these two bits in the correct location in loraBuffer

  if (temp &&
      (temp != 3)) {  /// no need to do the operation with 0, nor with B11
  ...
  if (temp != 3)
    ...
    if (pairCounter % 4 !=
        0) {  // do nothing if we have a zero in the topmost two bits already,
              // as this was end of character
      temp = temp << (2 * (3 - (pairCounter % 4)));
      loraTxBuffer[pairCounter / 4] |= temp;
    }
    pairCounter = 0;
...
void sendWithLora() {  // hand this string over as payload to the LoRA
                       // transceiver
  // send packet
  LoRa.beginPacket();
  LoRa.print(loraTxBuffer);
  LoRa.endPacket();
  if (morseState == loraTrx) LoRa.receive();
}
...
String CWwordToClearText(
    String cwword) {  // decode the Morse code character in cwword to clear text
  int ptr = 0;
  String result;
  result.reserve(40);
  String symbol;
...
String encodeProSigns(String& input) {
  /// clean up clearText   -   S <as>,  - A <ka> - N <kn> - K <sk> - H ch - V
  /// <ve>;
  input.replace("<as>", "S");
  input.replace("<ka>", "A");
  input.replace("<kn>", "N");
...
//// new buffer code: unpack when needed, to save buffer space. We just use 256
/// bytes of buffer, instead of 32k! / in addition to the received packet, we
/// need to store the RSSI as 8 bit positive number / (it is always between -20
/// and -150, so an 8bit integer is fine as long as we store it without sign as
/// an unsigned number) / the buffer is a 256 byte ring buffer with two
/// pointers: /   nextBuRead where the next packet starts for reading it out; is
/// incremented by l to get the next buffer read position /      you can read a
/// packet as long as the buffer is not empty, so we need to check bytesBuFree
/// before we read! if it is 256, the buffer is empty! /      with a read, the
/// bytesBuFree has to be increased by the number of bytes read /   nextBuWrite
/// where the next packet should be written; @write: /       increment
/// nextBuWrite by l to get new pointer; and decrement bytesBuFree by l to get
/// new free space / we also need a variable that shows how many bytes are free
///(not in use): bytesBuFree / if the next packet to be stored is larger than
/// bytesBuFree, it is discarded / structure of each packet: /    l:  1 uint8_t
/// length of packet /    r:  1 uint8_t rssi as a positive number /    d:  (var.
/// length) data packet as received by LoRa / functions: /    int
/// loRaBuWrite(int rssi, String packet): returns length of buffer if
/// successful. otherwise 0 / uint8_t loRaBuRead(uint8_t* buIndex): returns
/// length of packet, and index where to read in buffer by reference / bool
/// loRaBuReady(): true if there is something in the buffer, false otherwise /
/// example: /
///(somewhere else as global var: ourBuffer[256] /        uint8_t myIndex; /
/// uint8_t mylength; /        foo() { /          myLength =
/// loRaBuRead(&myIndex); /          if (myLength != 0) /
/// doSomethingWith(ourBuffer[myIndex], myLength); /        }

uint8_t loRaBuWrite(int rssi, String packet) {
  ////   int loRaBuWrite(int rssi, String packet): returns length of buffer if
  /// successful. otherwise 0 /   nextBuWrite where the next packet should be
  /// written; @write: /       increment nextBuWrite by l to get new pointer;
  /// and decrement bytesBuFree by l to get new free space
  uint8_t l, posRssi;
  ...
  for (int i = 0; i < packet.length();
       ++i) {  // do this for all chars in the packet
    loRaRxBuffer[nextBuWrite++] =
        packet[i];  // at end nextBuWrite is alread where it should be
  }
  byteBuFree -= l;
...
uint8_t loRaBuRead(uint8_t* buIndex) {
  ////    uint8_t loRaBuRead(uint8_t* buIndex): returns length of packet, and
  /// index where to read in buffer by reference
  uint8_t l;
  if (byteBuFree == 256)
    return 0;
  else {
...
void storePacket(int rssi, String packet) {  // whenever we receive something,
                                             // we just store it in our buffer
  if (loRaBuWrite(rssi, packet) == 0) Serial.println("LoRa Buffer full");
}
...
//// byte 1: header; first two bits are the protocol version (curently 01), plus
/// 6 bit paclet serial number (starting from random) / byte 2: first 6 bits are
/// wpm (must be between 5 and 50; values 00 - 04 and 51 to 63 are invalid), the
/// remaining 2 bits are already data payload!

uint8_t decodePacket(int* rssi, int* wpm, String* cwword) {
  uint8_t l, c, header = 0;
  uint8_t index = 0;

  l = loRaBuRead(&index);  // where are we in  the buffer, and how long is the
                           // total packet inkl. rssi byte?
  ...
  for (int i = 0; i < l; ++i) {  // decoding loop
    ...
    switch (i) {
      ...
      case 2:
        *wpm = (uint8_t)(
            c >>
            2);  // the first data byte contains the wpm info in the first six
                 // bits, and actual morse in the remaining two bits now take
                 // remaining two bits and store them in CWword as ASCII
        *cwword = (char)((c & B011) + 48);
        break;
      default
          :  // decode the rest of the packet; we do this for all but the first
             // byte  /// we need to handle end of word!!! therefore the break
        for (int j = 0; j < 4; ++j) {
          char cc = ((c >> 2 * (3 - j)) &
                     B011);  // we store them as ASCII characters 0,1,2,3 !
          if (cc != 3) {
            *cwword += (char)(cc + 48);
          } else
            break;
...
////////////////////////////
///// Routines for morse decoder - to a certain degree based on code by Hjalmar
/// Skovholm Hansen OZ1JHM - copyleft licence
////////////////////////////
...
// const float sampling_freq = 106000.0;
// const float target_freq = 698.0; /// adjust for your needs see above
// const int goertzel_n = 304; //// you can use:         152, 304, 456 or 608 -
// thats the max buffer reserved in checktone()//

void setupGoertzel() {  /// pre-compute some values that are compute-imntensive
                        /// and won't change anyway
  goertzel_n =
      (p_goertzelBandwidth == 0
           ? 152
           : 608);  // update Goertzel parameters depending on chosen bandwidth
  magnitudelimit_low =
      (p_goertzelBandwidth ? 160000 : 40000);  // values found by experimenting
...
bool straightKey() {  // return true if a straight key was closed, or a touch
                      // paddle touched
  if ((morseState == morseDecoder) &&
      ((!digitalRead(straightPin)) || leftKey || rightKey))
    return true;
  else
    return false;
}

bool checkTone() {  /// check if we have a tone signal at A6 with Gortzel's
                    /// algorithm, and apply some noise blanking as well the
                    /// result will be in globale variable filteredState we
                    /// return true when we detected a change in state, false
                    /// otherwise!
  ...
  uint16_t testData[1216];  /// buffer for freq analysis - max. 608 samples; you
                            /// could increase this (and n) to a max of 1216,
                            /// for sample time 10 ms, and bw 88 Hz
  ...
  if (straightKey()) {
    ...
    float magnitudeSquared =
        (Q1 * Q1) + (Q2 * Q2) -
        (Q1 * Q2 * coeff);  // we do only need the real part //
    magnitude = sqrt(magnitudeSquared);
    Q2 = 0;
    Q1 = 0;

    // Serial.println("Magnitude: " + String(magnitude) + " Limit: " +
    // String(magnitudelimit));   //// here you can measure magnitude for
    // setup..
    ...
    if (magnitude > magnitudelimit_low) {
      magnitudelimit = (magnitudelimit + ((magnitude - magnitudelimit) /
  ...
  switch (decoderState) {
    case INTERELEMENT_:
      if (checkTone()) {
        ...
        lacktime = 2.2;  ///  when high speeds we have to have a little more
                         ///  pause before new letter
        ...
        if (lowDuration > (lacktime * ditAvg)) {
          ...
          wpm = (p_wpm + (int)(7200 / (dahAvg + 3 * ditAvg))) /
    ...
    case INTERCHAR_:
      if (checkTone()) {
        ...
        lowDuration =
            millis() - startTimeLow;  // we record the length of the pause
        lacktime = 5.6;  ///  when high speeds we have to have a little more
...
void ON_() {  /// what we do when we just detected a rising flank, from low to
              /// high
...
void OFF_() {  /// what we do when we just detected a falling flank, from high
               /// to low
  unsigned long timeNow = millis();
...
void keyOut(bool on, bool fromHere, int f, int volume) {
  ...
  //// differentiate eternal (decoder, sometimes cw_generate) and internal
  ///(keyer, sometimes Cw-generate) side tones / key transmitter (and line-out
  /// audio if we are in a suitable mode)

  static bool intTone = false;
  static bool extTone = false;
  ...
  if (on) {
    if (fromHere) {
      ...
      extTone = true;
      extPitch = f;
      if (!intTone) pwmTone(extPitch, volume, false);
    }
  } else {  // key off
    if (fromHere) {
      intTone = false;
...
void audioLevelAdjust() {
  ...
  keyTx = true;
  keyOut(true, true, 698, 0);  /// we generate a side tone, f=698 Hz, also on
                               /// line-out, but with vol down on speaker
  while (true) {
    volButton.Update();
    if (volButton.clicks)
    ...
    for (i = 1; i < goertzel_n; ++i) {
      if (testData[i] < mini) mini = testData[i];
      if (testData[i] > maxi) maxi = testData[i];
    }
    int a, b, c;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
/// stuff using WiFi - ask for access point credentials, upload player file, do
/// OTA software update
///////////////////////////////////////////////////////////////////////////////////////////////////
...
void startAP() {
  ...
  server.on("/set", HTTP_GET, []() {
    ...
    server.send(200, "text/html",
                "Wifi Info updated - now restarting Morserino-32...");
...
void updateFirmware() {  /// start wifi client, web server and upload new binary
...
bool wifiConnect() {  // connect to local WLAN
  // Connect to WiFi network
  if (p_wlanSSID == "") return errorConnect(String("WiFi Not Conf"));
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    if ((millis() - wait) > 20000) return errorConnect(String("No WiFi"));
  }
...
void uploadFile() {
  ...
  /*
    server.on("/upload", HTTP_GET, []() {                 // if the client
    requests the upload page if (!handleFileUpload())                // send it
    if it exists server.send(404, "text/plain", "404: Not Found"); // otherwise,
  ...
  */
  server.on("/update", HTTP_POST,  // if the client posts to the upload page
            []() {
              ...
              ESP.restart();
            },  // Send status 200 (OK) to tell the client we are ready to
                // receive; when done, restart the ESP32
...
void handleFileUpload() {  // upload a new file to the SPIFFS
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    fsUploadFile =
        SPIFFS.open("/player.txt", "w");  // Open the file for writing in SPIFFS
    ...
    if (fsUploadFile) {       // If the file was successfully created
      ...
      // server.sendHeader("Location","/success.html");      // Redirect the
      // client to the success page server.send(303);
    } else {
...
String cleanUpText(String w) {  // all to lower case, and convert umlauts
  ...
  for (unsigned int i = 0; i < w.length(); ++i) {
    if (kochChars.indexOf(c = w.charAt(i)) != -1) result += c;
  }
  return result;
}
...
void skipWords(File fn,
               uint32_t count) {  /// just skip count words in open file fn
  while (count > 0) {
    getWord(fn);
    --count;
  }
